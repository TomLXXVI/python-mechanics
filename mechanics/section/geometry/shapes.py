...
class Circle(Shape):
    ...
    def __init__(
        ...
        """
        super().__init__(dim=Dimensions(radius=radius, diameter=2*radius))

    @property
    def area(self) -> Quantity:
        return pi * self.dim.radius**2

    @property
    def moment_of_inertia_xx(self) -> Quantity:
        return pi * self.dim.radius**4 / 4
    ...
    @property
    def product_of_inertia(self) -> Quantity:
        return Q_(0, self.dim.radius.units**4)

    @property
    def polar_moment_of_inertia(self) -> Quantity:
        return pi * self.dim.radius ** 4 / 2

    @property
    def principal_moments_of_inertia(self) -> tuple[Quantity, Quantity, Quantity]:
        I_max = self.moment_of_inertia_xx
        I_min = self.moment_of_inertia_yy
        theta = Q_(0, 'rad')
...
class Annulus(Shape):
    """
    Represents a ring (e.g. the cross-section of a tube or a hollow round
    shaft).
    """
    def __init__(
        self,
        ...
        self._inner_circle = Circle(radius_in)
        self._outer_circle = Circle(radius_out)

    @property
    def area(self) -> Quantity:
        A_outer = self._outer_circle.area
        A_inner = self._inner_circle.area
        A = A_outer - A_inner
        return A

    @property
    def moment_of_inertia_xx(self) -> Quantity:
        I_xx_outer = self._outer_circle.moment_of_inertia_xx
        I_xx_inner = self._inner_circle.moment_of_inertia_xx
        I_xx = I_xx_outer - I_xx_inner
        return I_xx

    @property
    def moment_of_inertia_yy(self) -> Quantity:
        I_yy_outer = self._outer_circle.moment_of_inertia_yy
        I_yy_inner = self._inner_circle.moment_of_inertia_yy
        I_yy = I_yy_outer - I_yy_inner
        return I_yy
    ...
    @property
    def polar_moment_of_inertia(self) -> Quantity:
        J_outer = self._outer_circle.polar_moment_of_inertia
        J_inner = self._inner_circle.polar_moment_of_inertia
        J = J_outer - J_inner
        return J
...
class Polygon(Shape):
    ...
    def centroid(self) -> tuple[Quantity, Quantity]:
        """Location of the centroid of the polygon with coordinates relative to
        the origin of the coordinate system used to specify the coordinates of
        the vertices.
        """
    ...
    def plot(self) -> LineChart:
        ...
        chart.add_xy_data(
            ...
            x1_values=x,
            y1_values=y,
            style_props={'color': 'black', 'marker': '*', 'lw': 2}
        )
        chart.add_xy_data(
    ...
    def __convert_to_float(pts: TPoints) -> list[tuple[float, float]]:
        pts = [tuple(p) for p in pts if not isinstance(p, tuple)]
