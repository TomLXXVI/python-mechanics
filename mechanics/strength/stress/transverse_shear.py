...
from mechanics.geometry.shapes import Shape, Polygon, HollowPolygon
from mechanics.geometry import LineSegment
...
class TransverseShear:
    """
    Determine the (maximum) shear stress in a cross-section associated with
    the resultant internal shear force.
    """
    def __init__(
        self,
        V: Quantity,
        ...
        """Creates a `TransverseShear` object.

        Parameters
        ----------
        V:
            Resultant internal shear force acting at the cross-section.
        shape:
    ...
    def flow(self, y: Quantity) -> Quantity:
        """Returns the value of the shear flow at vertical distance `y`
        relative to the centroid of the cross-section.

        Notes
        -----
        This method is only correct for vertical members of the cross-section.
        """
        if isinstance(self.shape, (Polygon, HollowPolygon)):
            pg = self._cut_polygon(y, self.shape)
            A_acc = pg.area
            _, y_acc = pg.centroid
    ...
    def tau(self, y: Quantity) -> Quantity:
        """Returns the transverse shear stress in the section at height `y`
        relative to the centroid of the cross-section.
        """
        if isinstance(self.shape, (Polygon, HollowPolygon)):
            try:
                ...
                return Q_(0.0, 'N / mm ** 2')
            A_acc = pg.area
            _, y_acc = pg.centroid
            Q = A_acc * abs(y_acc)
            I_xx = self.shape.moment_of_inertia_xx
            q = self.V * Q / I_xx
            t = self._determine_t(y, pg)
            if t is None:
                y = y - Q_(0.01, y.units)
                pg = self._cut_polygon(y, self.shape)
                t = self._determine_t(y, pg)
    ...
    def __order_segments(y: float, segments: list[LineSegment]):
        # Remove line segments that lay above (or below) the cut line at y:
        valid_segments = []
        for segment in segments:
    ...
    @staticmethod
    def __remove_duplicates(vertices: list[tuple[float, float]]) -> list[tuple[float, float]]:
    ...
    def _cut_polygon(self, y: Quantity, shape: Polygon | HollowPolygon) -> Polygon:
        """Returns the part of the polygon shape that lies above (y > 0) or
        below (y < 0) the cutting line at coordinate y.
        """
        y_mag = float(y.magnitude)
        segments = self.__create_line_segments(shape)
        intersections = self.__get_intersections(y_mag, segments)
        for i in range(len(segments)):
            ...
            for intersection in intersections:
                if segment.contains((intersection[0], intersection[1])):
                    ...
                    if segment.slope == 0:
                        pass
        segments = self.__order_segments(y_mag, segments)
        vertices = self.__get_vertices(y_mag, segments)
        vertices = self.__remove_duplicates(vertices)
        vertices = self.__convert_to_quantity(vertices, y.units)
        polygon = Polygon(vertices)
        return polygon

    def _determine_t(self, y: Quantity, cut_polygon: Polygon) -> Quantity | None:
        """Determines the width of the cross-section at the vertical distance y
        from the neutral axis (coordinate y).
        """
        # Get the horizontal line segments of the cut polygon:
        horizontal_segments = [
            segment
            for segment in self.__create_line_segments(cut_polygon)
            if segment.is_horizontal()
        ]
        # Determine t:
        if y.m >= 0:
            ...
            sum_of_lengths, total_length = self.__get_sum_of_lengths(segments_min_y1)
        ...
        if y.m < 0:
            ...
            if len(segments_max_y1) == 1:
                ...
                if self.__segments_do_overlap(segments_max_y1):
                    ...
                    sum_of_lengths, _ = self.__get_sum_of_lengths(segments_max_y1)
                    t = sum_of_lengths
                return Q_(t, y.units)
            else:
    ...
    @staticmethod
    def __get_sum_of_lengths(segments: list[LineSegment]) -> tuple[float, float]:
