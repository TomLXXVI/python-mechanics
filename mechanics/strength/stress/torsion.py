...
from mechanics.geometry.shapes import Shape, Circle, Annulus
...
class Torsion:
    """
    Determine the (maximum) shear stress in a circular or annular cross-section
    associated with the resultant internal torque.
    """
    def __init__(
        self,
        T: Quantity,
        ...
        if isinstance(shape, (Circle, Annulus)):
            ...
            raise ValueError('only circular or annular shapes are supported')
    ...
    def design(
        ...
        if issubclass(shape, Circle):
            r = (2 * abs(T) / (tau_allow * pi)) ** (1 / 3)
            return r
        if issubclass(shape, Annulus):
            r_o = cls.__calc_outer_radius(abs(T), tau_allow, t)
            return r_o
    ...
    def allowable_torque(
        ...
        """Returns the allowable torque that can be applied to a round shaft
    ...
    def tau(self, r: Quantity) -> Quantity:
        """Returns the shear stress at intermediate radius `r` of the round
        solid shaft or hollow tube.
        """
    ...
    @property
    def tau_max(self) -> Quantity:
        """Returns the maximum shear stress at the outer radius of the round
        solid shaft or hollow tube.
        """
