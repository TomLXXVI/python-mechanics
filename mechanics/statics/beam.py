...
class Beam2D:
    ...
    def get_reactions(self) -> dict[str, tuple[Force | None, Moment | None]]:
        """Get the solutions for the unknown reaction forces applied to the beam
        by the beam supports.

        Returns
        -------
        A dictionary of which the keys are the names given to the supports that
        exert reaction forces on the beam and of which the values are 2-tuples.
        The first element of a tuple is the reaction force (a `Force` object) or
        `None` if no reaction force is exerted by the support, and the second
        element is the reaction torque (a `Moment` object) or `None` if no
        reaction torque is exerted by the support.
        """
        ...
        # Get the moments about the z-axis of the known action forces:
        *_, tup_M_z_1 = zip(*[F.moment().components for F in forces])
        # Get the known action torques acting on the beam (torsion and bending):
        torques_a = self.__get_action_torques(x_min, x_max)
        if torques_a:
            tup_M_x, _, tup_M_z_2 = zip(*[T.components for T in torques_a])
        ...
        M_a_x = sum(M_x.to(self.__u_moment).m for M_x in tup_M_x)  # torsion
        M_a_z = sum(M_z.to(self.__u_moment).m for M_z in lst_M_z)  # bending
        ...
        M_r_x = sum([sup.M_x for sup in self.supports if sup.M_x is not None])
        M_r_z = sum([sup.M_z for sup in self.supports if sup.M_z is not None])
        # Add the moments of the reaction forces about the z-axis:
        M_r_z += sum([
        ...
        if M_r_x:
            M_x_bal = sp.Eq(M_r_x + M_a_x, 0)
            equations.append(M_x_bal)
        if M_r_z:
    ...
    def cut(self, x: Quantity, side: str = 'left') -> dict[str, Quantity]:
        ...
        if forces:
            tup_F_x, tup_F_y, _ = zip(*[F.components for F in forces])
            tup_M_x, _, tup_M_z = zip(*[F.moment().components for F in forces])
            F_a_x = sum(F_x.to(self.__u_force).m for F_x in tup_F_x)
            F_a_y = sum(F_y.to(self.__u_force).m for F_y in tup_F_y)
            M_a_z = sum(M_z.to(self.__u_moment).m for M_z in tup_M_z)  # bending
        else:
            F_a_x = 0.0
            F_a_y = 0.0
            M_a_z = 0.0
        ...
        # Get the x- and z-components of the known action torques and add them
        # to the sum of the moments of forces:
        if torques:
            tup_T_x, _, tup_T_z = zip(*[T.components for T in torques])
            M_a_x = sum(T_x.to(self.__u_moment).m for T_x in tup_T_x)   # torsion
            M_a_z += sum(T_z.to(self.__u_moment).m for T_z in tup_T_z)  # bending
        else:
            M_a_x = 0.0
        # Create the static equilibrium equations and solve for the components
        # of the unknown reaction forces:
        F_i_x, F_i_y = sp.symbols(['F_x', 'F_y'])
        M_i_x, M_i_z = sp.symbols(['M_x', 'M_z'])
        F_x_bal = sp.Eq(F_i_x, -F_a_x)
        F_y_bal = sp.Eq(F_i_y, -F_a_y)
        x = x.to(self.__u_pos).m
        M_x_bal = sp.Eq(M_i_x, -M_a_x)
        M_z_bal = sp.Eq(M_i_z + x * F_i_y, -M_a_z)
        eqs = [F_x_bal, F_y_bal, M_x_bal, M_z_bal]
    ...
    def __add_reactions(
    ...
    ) -> dict[str, tuple[Force | None, Moment | None]]:
        ...
        for name, d in reactions.items():
            ...
            if F_mag is not None:
                ...
                reactions_[name] = [None]
            M_z: float | None = d.get('M_z', None)
            if M_z is not None:
                M = self.__create_reaction_torque(name, M_z)
                self.action_torques.append(M)
                reactions_[name].append(M)
            else:
                reactions_[name].append(None)
            reactions_[name] = tuple(reactions_[name])
        return reactions_
    ...
    def __create_reaction_torque(self, name: str, M_z: float) -> Moment:
