...
class Circle(Polygon):
    ...
    def __init__(self, radius: Quantity) -> None:
        super().__init__(
            vertices=self.__create_vertices(radius),
            dim=Dimensions(radius=radius)
        )

    @staticmethod
    def __create_vertices(r: Quantity) -> list[tuple[Quantity, Quantity]]:
        theta_rng = [i for i in range(0, 91)]
        theta_rng = np.array(theta_rng) * np.pi / 180
        x_rng = r * np.round(np.cos(theta_rng), 9)
        x_rng = np.append(-x_rng[:-1], np.flip(x_rng))
        y_rng = np.sqrt(r ** 2 - x_rng ** 2)
        pts_pos = list(zip(x_rng, y_rng))
        pts_pos.reverse()
        pts_neg = list(zip(x_rng, -y_rng))
        pts = pts_pos + pts_neg
        return pts
